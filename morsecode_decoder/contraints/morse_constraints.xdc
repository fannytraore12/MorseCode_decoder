...
## Timing exceptions (added)
## Buttons are asynchronous (2-flop synchronized in RTL) and the LEDs / UART pin
## have no timing relationship to an external clock, so exclude them from I/O
## timing analysis. Internal paths are still fully checked against sys_clk_pin.
set_false_path -from [get_ports { btn[*] }]
set_false_path -to   [get_ports { led[*] led5_r led5_g led5_b ja[*] }]
